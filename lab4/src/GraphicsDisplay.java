...
// Перо для рисования графика
        float[] dash= {20,5,20,5,20,5,5,5,5,5,5,5};
...
canvas.setColor(Color.BLUE);
        ...
        for (Double[] point : graphicsData) {
            // Инициализировать квадрат как объект для представления маркера
            int sideLength = 10;  // You can adjust the side length as needed
            ...
            // Вершины квадрата
            int x1 = (int) (center.getX() - halfSide);
            int y1 = (int) (center.getY() - halfSide);
            int x2 = (int) (center.getX() + halfSide);
            int y2 = (int) (center.getY() + halfSide);



            // Рисуем контур квадрата с двумя диагоналями
            canvas.drawRect(x1, y1, sideLength, sideLength);
            canvas.drawLine(x1, y1, x2, y2);  // Diagonal from top-left to bottom-right
            canvas.drawLine(x1, y2, x2, y1);  // Diagonal from top-right to bottom-l
            ...
            String numStr = String.valueOf(n).replaceAll("[^\\d]", "");
            
            for (int i = 0; i < numStr.length() - 1; i++) {
               if (Character.toString(numStr.charAt(i)) > Character.toString(numStr.charAt(i + 1))) {
                   check =  false;
                   break;
               }
           }
           ...
           check =  true;
            ...
            if(check){
                canvas.setColor(Color.GREEN); // выбираем цвет для закраски
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        for (Double[] point : graphicsData) {
            ...
            boolean check = check(point[1]);


            if (check) {
                canvas.setColor(Color.GREEN);
                canvas.fillRect(x1, y1, sideLength, sideLength);
            }
            else{
                canvas.setColor(Color.BLUE);
            }
    ...
    private boolean check(double y) {
        String num = Double.toString(y);
        num = num.replaceAll("-", "");
        String[] parts = num.split("\\.");

        String intPart = parts[0];
        String fracPart = parts.length > 1 ? parts[1] : "";
        for (int i = 0; i < intPart.length() - 1; i++) {
            if (Integer.parseInt(Character.toString(intPart.charAt(i))) >= Integer.parseInt(Character.toString(intPart.charAt(i + 1)))) {
                return false;
            }
        }
        if(intPart.charAt(intPart.length()-1) >= fracPart.charAt(0)){
            return false;
        }
        for (int i = 0; i < fracPart.length() - 3; i++) {
            if (Integer.parseInt(Character.toString(fracPart.charAt(i))) >= Integer.parseInt(Character.toString(fracPart.charAt(i + 1)))) {
                return false;
            }
        }
        return true;
